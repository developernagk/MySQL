...
# : 데이터베이스를 삭제하는 기능, 해당 작업은 실행 후 되돌릴 수 없음
# 기본형태
...
# : create table table_name(
	 ...
	 # ...
# )
# optionals: 컬럼에 적용할 선택적 특성 - not null 지정, 기본값 설정 등
...
# : 테이블의 구조 - 테이블에 정의된 컬럼, 데이터 타입, 키 정보 등을 조회하는데 사용
use example;
-- 왼쪽  네비게이터의 Shcemas 필드의 데이터베이스명을 더블클릭
# 기본형태
# : describe table_name;
describe `students`;
# Field(각 컬럼의 이름), Type(각 컬럼의 데이터 타입), Null(Null 허용 여부), Key(각 컬럼의 제약사항-키), Default(기본값 지정), Extra(제약사항-추가옵션)
desc `students`;

# 3. 테이블 수정()
# : 이미 존재하는 테이블의 구조를 변경하는데 사용
# : 컬럼을 추가, 수정, 삭제 또는 제약조건을 추가, 수정, 삭제
...
# drop 문은 해당 삭제하고자하는 형태가 존재하지 않을 경우 오류
# : if exists 사용 시 존재할 때만 삭제하기 때문에 오류 일어나지 않음
drop table if exists `lectures`; -- 오류 발생 시 해당 위치부터 그 아래의 코드 실행이 안 됨
drop table if exists `students`;
drop table if exists `integer`;

# 5. 테이블 목록 조회(show tables)
# : 모든 데이터베이스에 있는 모든 테이블 목록을 조회
