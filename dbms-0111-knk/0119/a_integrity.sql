...
# ex) 학생 - 수강: 강의 목록 테이블에서 수강 기록은 특정 학생을 참조

# 3. 도메인 무결성
# : 테이블이 각 필드가 정의된 도메인에 맞는 값을 가져야 함을 뜻함
# : 데이터타입, 포맷, 범위 등이 적절하게 유지
# 도메인: 데이터타입, 길이, 가능한 값의 범위
# ex) 은행 계좌에 입금되는 금액 - 양의 정수만을 허용하는 unsigned 설정

# 4. 사용자 정의 무결성
# : 특정 애플리케이션의 규칙에 따라 정의
# : 구현하고자 하는 비즈니스 규칙이나 애플리케이션의 특정 요구 사항을 반영
# ex) A 홈페이지 - 회원가입 시 비밀번호 설정을 6~8자 이내, 영문자,숫자,특수문자 포함