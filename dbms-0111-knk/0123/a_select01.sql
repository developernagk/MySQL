...
# member 테이블 - area_code가 BUSAN이면서 grade이 "Silver" 
select
	*
from 
	`korea_db`.`member`
...
# 지역코드(area_code)가 SEOUL이거나 BUSAN인 회원을 조회
select 
	*
from 
	`korea_db`.`member`
...
# : null 여부를 확인하는 키워드 - is null, is not null
# is null
...
# points가 200 이상이면서 400 이하인 데이터 추출
...
# 2023년에 회원가입한 모든 회원 조회
select * from `member` where year(join_date)  = 2023;

# 시간 일치: hour(), minute(), second()
