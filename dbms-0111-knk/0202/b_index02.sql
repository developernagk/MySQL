...
# 데이터를 읽을 때: 데이터의 건수는 의미 없음
# 몇 개의 페이지를 읽었느냐로 효율성을 판단함

# 균형 트리의 페이지 분할
# 인덱스 구성 시 데이터 변경 작업
# : insert, update, delete 사용 시 성능 저하
# >> 페이지 분할 작업 수행

#! 인덱스 구조
# : 인덱스를 생성하면 왜 데이터가 정렬되는지 어떤 인덱스가 더 효율적인지 
...
# 위의 cluster 테이블은 1개의 루트 페이지, 3개의 리프(데이터) 페이지로 구성
...
# 보조 인덱스의 경우 데이터 페이지가 바뀌지 않음
# >> 별도의 공간에 인덱스 페이지 생성
# 보조 인덱스의 데이터 위치는: 페이지 번호 + #위치
# 클러스터형 인덱스가 보조 인덱스보다 속도가 좀 더 빠름